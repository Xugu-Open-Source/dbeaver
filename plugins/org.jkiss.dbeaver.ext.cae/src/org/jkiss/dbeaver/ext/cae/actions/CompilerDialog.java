 ...
 *
 */
public class CompilerDialog extends BaseDialog
{
    private static final Log log = Log.getLog(CompilerDialog.class);

    private static final int COMPILE_ID = 1000;
    private static final int COMPILE_ALL_ID = 1001;

    private java.util.List<SourceObject> compileUnits;
    private TableViewer unitTable;

    private ObjectCompilerLogViewer compileLog;


    public CompilerDialog(Shell shell, java.util.List<SourceObject> compileUnits)
    {
        super(shell, UIMessages.views_compiler_dialog_title, null);
    ...
    protected Composite createDialogArea(Composite parent)
    {
...

        {
            ...
            ViewerColumnController columnController = new ViewerColumnController("CompilerDialog", unitTable);
            columnController.addColumn(UIMessages.views_compiler_dialog_column_name, null, SWT.NONE, true, true, new CellLabelProvider() {
                @Override
            ...
            columnController.addColumn(UIMessages.views_compiler_dialog_column_type, null, SWT.NONE, true, true, new CellLabelProvider() {
                @Override
                public void update(ViewerCell cell)
                {
                    DBSObject unit = (DBSObject) cell.getElement();
            ...
            unitTable.addDoubleClickListener(event -> {
                ...
                if (!selection.isEmpty()) {
                    SourceObject unit = (SourceObject) selection.getFirstElement();
    ...
    protected void createButtonsForButtonBar(Composite parent)
    {
        ...
        createButton(parent, COMPILE_ID, UIMessages.views_compiler_dialog_button_compile, false).setEnabled(false);
		createButton(parent, COMPILE_ALL_ID, UIMessages.views_compiler_dialog_button_compile_all, true);
    ...
    {
        final List<SourceObject> toCompile;
    ...
    private void performCompilation(DBRProgressMonitor monitor, List<SourceObject> units)
    {
        compileLog.layoutLog();
        for (SourceObject unit : units) {
            if (monitor.isCanceled()) {
                break;
            }
            final String message = NLS.bind(UIMessages.views_compiler_dialog_message_compile_unit, unit.getSourceType().name(), unit.getName());
            ...
            compileLog.info(!success ? UIMessages.views_compiler_dialog_message_compilation_error : UIMessages.views_compiler_dialog_message_compilation_success);
            compileLog.info("");  
        }
