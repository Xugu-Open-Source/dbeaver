...
abstract class XuguWizardPageSettings<WIZARD extends AbstractNativeToolWizard> extends AbstractNativeToolWizardPage<WIZARD>
{

    XuguWizardPageSettings(WIZARD wizard, String title)
    {
        super(wizard, title);
    }
    ...
    public void createSecurityGroup(Composite parent)
    {
        ...
        if (connectionInfo != null) {
            Composite securityGroup = UIUtils.createTitledComposite(parent, XuguUIMessages.tools_db_export_wizard_page_settings_security_group, 3, GridData.HORIZONTAL_ALIGN_BEGINNING, 0);
            Label infoLabel = new Label(securityGroup, SWT.NONE);
            infoLabel.setText(NLS.bind(XuguUIMessages.tools_db_export_wizard_page_settings_security_label_info, connectionInfo.getUserName()));
            GridData gd = new GridData(GridData.FILL_HORIZONTAL);
            gd.horizontalSpan = 3;
            infoLabel.setLayoutData(gd);
            Button authButton = new Button(securityGroup, SWT.PUSH);
            authButton.setText(XuguUIMessages.tools_db_export_wizard_page_settings_security_button_auth);
            authButton.addSelectionListener(new SelectionAdapter() {
                @Override
                public void widgetSelected(SelectionEvent e) {
                    BaseAuthDialog authDialog = new BaseAuthDialog(getShell(), XuguUIMessages.tools_db_export_wizard_page_settings_auth_title, false, true);
                    ...
                    authDialog.setSavePasswordText(XuguUIMessages.tools_db_export_wizard_page_settings_auth_save_password_checkbox);
                    authDialog.setSavePasswordToolTipText(XuguUIMessages.tools_db_export_wizard_page_settings_auth_save_password_checkbox_tip);
            ...
            resetButton.setText(XuguUIMessages.tools_db_export_wizard_page_settings_security_button_reset);
...
//
//                Button overrideCredentials = UIUtils.createCheckbox(securityGroup, XuguUIMessages.tools_db_export_wizard_page_settings_security_checkbox_override_host_credentials, settings.isOverrideCredentials());
//                overrideCredentials.setToolTipText(XuguUIMessages.tools_db_export_wizard_page_settings_security_checkbox_override_host_credentials_tip);
