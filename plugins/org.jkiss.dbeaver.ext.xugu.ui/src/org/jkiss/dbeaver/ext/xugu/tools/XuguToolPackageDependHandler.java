 ...
 * 分析 XuguDB 中表对象的依赖关系
 */
public class XuguToolPackageDependHandler extends XuguToolAbstractHandler {
    ...
    public void XuguToolSubclassReach(IWorkbenchWindow window, IWorkbenchPart activePart, Collection<DBSObject> objects) {
        org.jkiss.dbeaver.ext.xugu.model.Package firstPackage =
            (org.jkiss.dbeaver.ext.xugu.model.Package) (((ArrayList<?>) objects).getFirst());
        String schemaName = firstPackage.getSchema().getName();
        String name = firstPackage.getName();
        ...
        try (Connection currentConn = firstPackage.getDataSource().getConnection()) {
            setData(currentConn, firstPackage, table1, DEPEND_OBJECT1);
            setData(currentConn, firstPackage, table2, DEPEND_OBJECT2);
    ...
    /**
     *
     * @param firstPackage    当前数据
     * @param table           tab页表格选择
     * @param dependObjectSQL 查询对象之间依赖关系sql
     */
    private static void setData(Connection currentConn, org.jkiss.dbeaver.ext.xugu.model.Package firstPackage, Table table,
                                String dependObjectSQL) {

        Vector<Vector<Integer>> allDependObjects = new Vector<>();
        long schemaId = firstPackage.getSchema().getId();
        long packId = firstPackage.getPackId();
        String roleFlag = firstPackage.getSchema().getRoleFlag().toLowerCase(Locale.ENGLISH);
        String[] parameters = {Long.toString(schemaId), Long.toString(packId), roleFlag};
