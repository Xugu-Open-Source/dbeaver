 ...
 * 分析 XuguDB 中表对象的依赖关系
 */
public class XuguToolViewDependHandler extends XuguToolAbstractHandler {
    ...
    public void XuguToolSubclassReach(IWorkbenchWindow window, IWorkbenchPart activePart, Collection<DBSObject> objects) {
        org.jkiss.dbeaver.ext.xugu.model.View firstView = (org.jkiss.dbeaver.ext.xugu.model.View) (((ArrayList<?>) objects).getFirst());
        String schemaName = firstView.getSchema().getName();
        String name = firstView.getName();
        ...
        try (Connection currentConn = firstView.getDataSource().getConnection()) {
            setData(currentConn, firstView, table1, DEPEND_OBJECT1);
            setData(currentConn, firstView, table2, DEPEND_OBJECT2);
    ...
    /**
     *
     * @param firstView      当前数据
     * @param table           tab页表格选择
     * @param dependObjectSQL 查询对象之间依赖关系sql
     */
    private static void setData(Connection currentConn, org.jkiss.dbeaver.ext.xugu.model.View firstView, Table table,
                                String dependObjectSQL) {

        Vector<Vector<Integer>> allDependObjects = new Vector<>();
        long schemaId = firstView.getSchema().getId();
        int id = firstView.getId();
        String roleFlag = firstView.getSchema().getRoleFlag().toLowerCase(Locale.ENGLISH);
        String[] parameters = {Long.toString(schemaId), Integer.toString(id), roleFlag};
