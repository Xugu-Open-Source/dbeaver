...
class XuguExportWizardPageObjects extends XuguWizardPageSettings<XuguExportWizard>
{

    private Table catalogTable;
    private Table tablesTable;
    private Map<Schema, Set<org.jkiss.dbeaver.ext.xugu.model.Table>> checkedObjects = new HashMap<>();

    private Schema curCatalog;
    private Button exportViewsCheck;

    protected XuguExportWizardPageObjects(XuguExportWizard wizard)
    {
        super(wizard, XuguUIMessages.tools_db_export_wizard_page_objects_dialog_wizard_title);
        setTitle(XuguUIMessages.tools_db_export_wizard_page_objects_dialog_title_choose_objects);
        setDescription(XuguUIMessages.tools_db_export_wizard_page_objects_dialog_description);
    }
    ...
    public void createControl(Composite parent)
    {
        ...
        Composite objectsGroup = UIUtils.createTitledComposite(composite, XuguUIMessages.tools_db_export_wizard_page_settings_group_objects, 1, GridData.FILL_HORIZONTAL, 0);
...

        {
            ...
            catalogTable.addListener(SWT.Selection, event -> {
                ...
                if (item != null) {
                    Schema catalog = (Schema) item.getData();
...

        {
            ...
            exportViewsCheck = UIUtils.createCheckbox(buttonsPanel, XuguUIMessages.tools_db_export_wizard_page_settings_group_show_views, false);
    ...
    private void loadSettings() {
        ...
        Set<Schema> activeCatalogs = new LinkedHashSet<>();
        for (DatabaseExportInfo info : wizard.getSettings().getExportObjects()) {
            activeCatalogs.add(info.getSchemaCache());
            dataSource = info.getSchemaCache().getDataSource();
            if (!CommonUtils.isEmpty(info.getTables())) {
                Set<org.jkiss.dbeaver.ext.xugu.model.Table> tables =
                        checkedObjects.computeIfAbsent(info.getSchemaCache(), k -> new HashSet<>());
                for (org.jkiss.dbeaver.ext.xugu.model.Table table : info.getTables()) {
                    tables.add(table);
                    if (table.isView()) {
                        hasViews = true;
                    }
        ...
        if (dataSource != null) {
            ...
            for (Schema catalog : dataSource.schemaCache.getCachedObjects()) {
...
//                }
    ...
    private void updateCheckedTables() {
        Set<org.jkiss.dbeaver.ext.xugu.model.Table> checkedTables = new HashSet<>();
        TableItem[] tableItems = tablesTable.getItems();
        for (TableItem item : tableItems) {
            if (item.getChecked()) {
                checkedTables.add((org.jkiss.dbeaver.ext.xugu.model.Table) item.getData());
            }
        }
    ...
    private boolean isChecked(Schema catalog) {
    ...
    private void loadTables(final Schema catalog) {
        if (catalog != null) {
            curCatalog = catalog;
        }
        if (curCatalog == null) {
        ...
        final Set<org.jkiss.dbeaver.ext.xugu.model.Table> checkedObjects = this.checkedObjects.get(curCatalog);
        new AbstractJob("Load '" + curCatalog.getName() + "' tables") {
            ...
            protected IStatus run(DBRProgressMonitor monitor) {
                try {
                    final List<BaseTable> objects = new ArrayList<>();
...
//                    }
    ...
    public void saveState() {
        List<DatabaseExportInfo> objects = wizard.getSettings().getExportObjects();
        objects.clear();
        for (TableItem item : catalogTable.getItems()) {
            if (item.getChecked()) {
                Schema catalog = (Schema) item.getData();
                DatabaseExportInfo info = new DatabaseExportInfo(catalog, checkedObjects.get(catalog));
                objects.add(info);
            }
        }
    }
