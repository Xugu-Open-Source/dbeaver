 ...
 * 分析 XuguDB 中表对象的依赖关系
 */
public class XuguToolTableDependHandler extends XuguToolAbstractHandler {
    ...
    public void XuguToolSubclassReach(IWorkbenchWindow window, IWorkbenchPart activePart, Collection<DBSObject> objects) {
        org.jkiss.dbeaver.ext.xugu.model.Table firstTable = (org.jkiss.dbeaver.ext.xugu.model.Table) (((ArrayList<?>) objects).getFirst());
        String schemaName = firstTable.getSchema().getName();
        String name = firstTable.getName();
        ...
        try (Connection currentConn = firstTable.getDataSource().getConnection()) {
            setData(currentConn, firstTable, table1, DEPEND_OBJECT1);
            setData(currentConn, firstTable, table2, DEPEND_OBJECT2);
    ...
    /**
     *
     * @param firstTable      当前数据
     * @param table           tab页表格选择
     * @param dependObjectSQL 查询此对象依赖的对象、查询依赖此对象的对象
     */
    private static void setData(Connection currentConn, org.jkiss.dbeaver.ext.xugu.model.Table firstTable, Table table,
                                String dependObjectSQL) {

        Vector<Vector<Integer>> allDependObjects = new Vector<>();
        int schemaId = firstTable.getSchemaId();
        int tableId = firstTable.getTableId();
        String roleFlag = firstTable.getSchema().getRoleFlag().toLowerCase(Locale.ENGLISH);
        String[] parameters = {Integer.toString(schemaId), Integer.toString(tableId), roleFlag};
