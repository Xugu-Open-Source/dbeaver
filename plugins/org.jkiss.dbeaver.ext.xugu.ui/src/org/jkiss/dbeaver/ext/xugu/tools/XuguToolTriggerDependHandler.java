 ...
 * 分析 XuguDB 中表对象的依赖关系
 */
public class XuguToolTriggerDependHandler extends XuguToolAbstractHandler {
    ...
    public void XuguToolSubclassReach(IWorkbenchWindow window, IWorkbenchPart activePart, Collection<DBSObject> objects) {
        org.jkiss.dbeaver.ext.xugu.model.Trigger firstTrigger = (org.jkiss.dbeaver.ext.xugu.model.Trigger) (((ArrayList<?>) objects).getFirst());
        String schemaName = firstTrigger.getSchema().getName();
        String name = firstTrigger.getName();
        ...
        try (Connection currentConn = firstTrigger.getDataSource().getConnection()) {
            setData(currentConn, firstTrigger, table1, DEPEND_OBJECT1);
            setData(currentConn, firstTrigger, table2, DEPEND_OBJECT2);
    ...
    /**
     *
     * @param firstTrigger      当前数据
     * @param table           tab页表格选择
     * @param dependObjectSQL 查询对象之间依赖关系sql
     */
    private static void setData(Connection currentConn, org.jkiss.dbeaver.ext.xugu.model.Trigger firstTrigger, Table table,
                                String dependObjectSQL) {

        Vector<Vector<Integer>> allDependObjects = new Vector<>();
        long schemaId = firstTrigger.getSchema().getId();
        int triggerId = firstTrigger.getTrigId();
        String roleFlag = firstTrigger.getSchema().getRoleFlag().toLowerCase(Locale.ENGLISH);
        String[] parameters = {Long.toString(schemaId), Integer.toString(triggerId), roleFlag};
