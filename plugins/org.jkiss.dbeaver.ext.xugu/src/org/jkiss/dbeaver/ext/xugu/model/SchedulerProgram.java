...
public class SchedulerProgram extends BaseSchemaObject {
	...
	SchedulerProgram(Schema schema, ResultSet dbResult) {
	...
	@Association
	public Collection<SchedulerProgramArgument> getArguments(DBRProgressMonitor monitor) throws DBException {
		return argumentsCache.getAllObjects(monitor, this);
	}

	static class ArgumentsCache extends JDBCObjectCache<SchedulerProgram, SchedulerProgramArgument> {
		@Override
		protected JDBCStatement prepareObjectsStatement(@NotNull JDBCSession session, @NotNull SchedulerProgram program)
				throws SQLException {
			JDBCPreparedStatement dbStat = session.prepareStatement("SELECT * FROM SYS.ALL_SCHEDULER_PROGRAM_ARGS "
					+ "WHERE OWNER=? AND PROGRAM_NAME=? " + "ORDER BY ARGUMENT_POSITION");
		...
		@Override
		protected SchedulerProgramArgument fetchObject(@NotNull JDBCSession session, @NotNull SchedulerProgram program,
				@NotNull JDBCResultSet resultSet) throws SQLException, DBException {
			return new SchedulerProgramArgument(program, resultSet);
		}
	}
}
