...
public class StructureAssistant implements DBSStructureAssistant {
	...
	@NotNull
	@Override
	public List findObjectsByMask(DBRProgressMonitor monitor, DBCExecutionContext executionContext,
			DBSObject parentObject, DBSObjectType[] objectTypes, String objectNameMask, boolean caseSensitive,
			boolean globalSearch, int maxResults) throws DBException {
		Schema schema = parentObject instanceof Schema ? (Schema) parentObject : null;
		try (JDBCSession session = DBUtils.openMetaSession(monitor, dataSource, "Find objects by name")) {
			...
			if (ArrayUtils.contains(objectTypes, ObjectType.CONSTRAINT, ObjectType.FOREIGN_KEY)
					&& objects.size() < maxResults) {
				// Search constraints
			...
			final Schema activeSchema = dataSource.getDefaultObject();
			objects.sort((o1, o2) -> {
	...
	private void findConstraintsByMask(JDBCSession session, final Schema schema, String constrNameMask,
			DBSObjectType[] objectTypes, int maxResults, List<DBSObjectReference> objects)
			...
			try (JDBCResultSet dbResult = dbStat.executeQuery()) {
				...
				while (dbResult.next() && tableNum-- > 0) {
					...
					objects.add(new AbstractObjectReference(constrName,
							...
							type == DBSEntityConstraintType.FOREIGN_KEY ? ObjectType.FOREIGN_KEY
	...
	private void searchAllObjects(final JDBCSession session, final Schema schema, String objectNameMask,
			DBSObjectType[] objectTypes, boolean caseSensitive, int maxResults, List<DBSObjectReference> objects)
			throws SQLException, DBException {
		StringBuilder objectTypeClause = new StringBuilder(100);
		final List<ObjectType> objectTypeList = new ArrayList<>(objectTypes.length + 2);
