...
public class DataTypeMethod extends BaseDataTypeMember implements DBSEntityMethod, DBSParametrizedObject {
	...
	private DataTypeModifier resultTypeMod;
	private final ParameterCache parameterCache;

	public DataTypeMethod(DataType dataType) {
		super(dataType);
		this.parameterCache = new ParameterCache();
	}

	public DataTypeMethod(DBRProgressMonitor monitor, DataType dataType, ResultSet dbResult) {
		...
		this.flagFinal = JDBCUtils.safeGetBoolean(dbResult, "FINAL", Constants.YES);
		this.flagInstantiable = JDBCUtils.safeGetBoolean(dbResult, "INSTANTIABLE", Constants.YES);
		this.flagOverriding = JDBCUtils.safeGetBoolean(dbResult, "OVERRIDING", Constants.YES);
		...
		if (!CommonUtils.isEmpty(resultTypeName)) {
			this.resultType = DataType.resolveDataType(resultTypeName);
			this.resultTypeMod = DataTypeModifier
	...
	public DataTypeModifier getResultTypeMod() {
	...
	@Override
	@Association
	public Collection<DataTypeMethodParameter> getParameters(DBRProgressMonitor monitor) throws DBException {
		return parameterCache == null ? null : parameterCache.getAllObjects(monitor, this);
	}

	private class ParameterCache extends JDBCObjectCache<DataTypeMethod, DataTypeMethodParameter> {
		@Override
		protected JDBCStatement prepareObjectsStatement(@NotNull JDBCSession session, @NotNull DataTypeMethod owner)
				throws SQLException {
			...
			DataType dataType = getOwnerType();
		...
		@Override
		protected DataTypeMethodParameter fetchObject(@NotNull JDBCSession session, @NotNull DataTypeMethod owner,
				@NotNull JDBCResultSet resultSet) throws SQLException, DBException {
			return new DataTypeMethodParameter(session.getProgressMonitor(), DataTypeMethod.this, resultSet);
		}
	}
}
