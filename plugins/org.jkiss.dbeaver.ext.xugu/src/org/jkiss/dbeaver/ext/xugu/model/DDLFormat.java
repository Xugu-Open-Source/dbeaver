...
public enum DDLFormat {
	...
	private static final Log log = org.jkiss.dbeaver.Log.getLog(DDLFormat.class);

	private DDLFormat(String title, boolean showStorage, boolean showSegments, boolean showTablespace) {
	...
	public static DDLFormat getCurrentFormat(DataSource dataSource) {
		String ddlFormatString = dataSource.getContainer().getPreferenceStore()
				.getString(Constants.PREF_KEY_DDL_FORMAT);
		if (!CommonUtils.isEmpty(ddlFormatString)) {
			try {
				return DDLFormat.valueOf(ddlFormatString);
			} catch (IllegalArgumentException e) {
				log.error(e);
			}
		}
		return DDLFormat.FULL;
	}
}
