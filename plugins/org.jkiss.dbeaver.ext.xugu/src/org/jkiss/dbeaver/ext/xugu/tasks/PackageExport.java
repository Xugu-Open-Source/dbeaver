	...
	public void execute(IWorkbenchWindow window, IWorkbenchPart activePart, Collection<DBSObject> objects)
		...
		// TODO Auto-generated method stub
				...
				if(objects.iterator().hasNext()) {
					Package  object = (Package)objects.iterator().next();
					packageNameString = object.getName();
					Schema schema = (Schema)(((DBSObject) object).getParentObject()); 
				...
				if(schemaNameString!=""&&schemaNameString!=null) {
					    outputFolder = new File(RuntimeUtils.getUserHomeDir().getAbsolutePath()+"\\"+schemaNameString+"_package_"+fileName+".sql");
				}
				 byte[] b = new Parsing().getDatabaseObjectDDL(currentConnection,databaseNameString,schemaNameString,"package",packageNameString, Parsing.TableType.ALL).getBytes();
